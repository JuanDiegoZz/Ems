...
import { isDeliverablePolice, validateCivilDelivery, type CivilDeliveryInput } from "../lib/deliveries/validation.ts";
import { createSupabaseAdminClient } from "../lib/supabase/admin.ts";
import { dispatchDelivery } from "./discord-deliveries.ts";
import { startOfDayInTimeZone } from "../lib/time/date.ts";

export type DeliveryRecord = { id: string; client_request_id: string; person_id: string; delivered_by: string; type: "civil" | "police"; quantity_label: string; occurred_at: string; status: "pending" | "sent" | "failed"; discord_message_id?: string | null; discord_error?: string | null; sent_at?: string | null };
...
export async function createCivilDelivery(input: CivilDeliveryInput) {
  ...
  const person = await client.from("people").select("id, type, archived_at").eq("id", delivery.personId).maybeSingle<{ id: string; type: "civil" | "police"; archived_at: string | null }>();
  if (!person.data || person.data.type !== "civil" || person.data.archived_at) throw new Error("El civil no está disponible para una entrega");
...
export async function createPoliceDelivery(input: CivilDeliveryInput) {
  const profile = await requireActiveProfile();
  const delivery = validateCivilDelivery(input);
  const client = createSupabaseAdminClient();
  ...
  const { data, error } = await client.from("deliveries").insert({ client_request_id: delivery.clientRequestId, person_id: delivery.personId, delivered_by: profile.id, type: "police", quantity_label: delivery.quantityLabel, occurred_at: new Date().toISOString(), status: "pending" }).select("*").single<DeliveryRecord>();
  if (!error && data) { const result = await dispatchDelivery(data.id); return { ...result.delivery, discordStatus: result.discordStatus, discordError: result.discordError }; }
  if (error?.code === "23505") { const existing = await client.from("deliveries").select("*").eq("client_request_id", delivery.clientRequestId).maybeSingle<DeliveryRecord>(); if (existing.data) { const result = await dispatchDelivery(existing.data.id); return { ...result.delivery, discordStatus: result.discordStatus, discordError: result.discordError }; } }
