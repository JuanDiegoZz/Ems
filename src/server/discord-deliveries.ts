...
import { perfTimer } from "../lib/perf.ts";
...
export async function dispatchDelivery(deliveryId: string) {
  const done = perfTimer("dispatch Discord delivery");
  try {
  ...
  try {
    const [person, profile] = await Promise.all([
      client.from("people").select("display_name, badge_number, ine_path, badge_path").eq("id", delivery.person_id).single<{ display_name: string; badge_number: string | null; ine_path: string; badge_path: string | null }>(),
      client.from("profiles").select("rp_name").eq("id", delivery.delivered_by).single<{ rp_name: string }>(),
    ]);
    ...
    const inePromise = client.storage.from("rp-documents").download(person.data.ine_path);
    let content = "";
    if (delivery.type === "police") {
      ...
      const [ine, badge] = await Promise.all([inePromise, client.storage.from("rp-documents").download(person.data.badge_path)]); if (ine.error || !ine.data) throw new Error("fallo al descargar INE"); if (badge.error || !badge.data) throw new Error("fallo al descargar placa");
    ...
    } else { const ine = await inePromise; if (ine.error || !ine.data) throw new Error("fallo al descargar INE"); const ineType = ine.data.type || "image/jpeg"; attachments.push({ blob: ine.data, contentType: ineType, filename: `ine-${safeName(person.data.display_name)}.${ineType.split("/")[1] || "jpg"}` }); content = buildCivilContent(profile.data.rp_name, person.data.display_name, formatDiscordDate(delivery.occurred_at), delivery.quantity_label); }
