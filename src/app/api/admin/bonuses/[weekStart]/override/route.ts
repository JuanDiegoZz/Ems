...
import { overrideBonusResult } from "@/server/bonus-runs";
...
export async function POST(request: Request, { params }: { params: Promise<{ weekStart: string }> }) {
  ...
  try { return NextResponse.json(await overrideBonusResult({ weekStart: (await params).weekStart, profileId: body?.profileId, amount: body?.amount, reason: body?.reason }), { headers: { "Cache-Control": "private, no-store" } }); }
