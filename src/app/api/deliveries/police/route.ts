...
export async function POST(request: Request) { try { return NextResponse.json(await createPoliceDelivery(await request.json()), { status: 201 }); } catch (error) { if (error instanceof DailyFreeKitConfirmationError) return NextResponse.json({ code: error.code, error: error.message }, { status: 409 }); return NextResponse.json({ error: error instanceof Error ? error.message : "No se pudo registrar la entrega" }, { status: 400 }); } }
