...
export function AppShell({ profile, children }: { profile: Profile; children: ReactNode }) {
  ...
  const nav = [...links, ...(profile.role === "admin" ? [{ href: "/admin/staff", label: "Personal EMS", icon: "shield" as IconName }, { href: "/admin/ems-performance", label: "Rendimiento EMS", icon: "activity" as IconName }, { href: "/admin/settings", label: "Configuración", icon: "settings" as IconName }] : [])];
