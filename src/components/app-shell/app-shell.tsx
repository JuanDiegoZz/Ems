...
export function AppShell({ profile, children }: { profile: Profile; children: ReactNode }) {
  ...
  const nav = [...links, { href: "/prices", label: "Precios", icon: "badge" as IconName }, ...(profile.role === "admin" ? [{ href: "/admin/staff", label: "Personal EMS", icon: "shield" as IconName }, { href: "/admin/bonuses", label: "Bonos semanales", icon: "badge" as IconName }, { href: "/admin/ems-performance", label: "Rendimiento EMS", icon: "activity" as IconName }, { href: "/admin/settings", label: "Configuración", icon: "settings" as IconName }] : [])];
