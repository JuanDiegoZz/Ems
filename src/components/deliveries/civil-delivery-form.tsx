...
export function CivilDeliveryForm({ rpName, timeZone }: { rpName: string; timeZone: string }) {
  ...
  return <div className="grid gap-5"><div className="glass-card grid gap-2 p-4"><input className="field-input" placeholder="Buscar civil por nombre o apellido" value={query} onChange={(event) => setQuery(event.target.value)} />{status === "searching" && <p className="text-xs text-[var(--muted)]">Buscando…</p>}</div>{people.length > 0 && <div className="grid gap-3">{people.map((person) => <button className={`glass-card p-4 text-left ${selected?.id === person.id ? "ring-2 ring-blue-500" : ""}`} type="button" key={person.id} onClick={() => setSelected(person)}><p className="font-semibold">{person.display_name}</p><p className="text-sm text-[var(--muted)]">Civil · INE disponible · Registrado {formatDate(person.created_at, timeZone)}</p></button>)}</div>}{people.length === 0 && query && status !== "searching" && <div className="glass-card p-5 text-sm">No encontramos a esta persona. <Link className="font-semibold text-blue-300" href="/people/new?type=civil&returnTo=/deliveries/civil">Registrar nuevo civil</Link></div>}{selected && <form onSubmit={submit} className="glass-card grid gap-4 p-6"><div><p className="text-sm text-[var(--muted)]">Civil seleccionado</p><p className="text-xl font-semibold">{selected.display_name}</p><p className="text-sm text-[var(--muted)]">INE disponible · <DocumentViewer personId={selected.id} personName={selected.display_name} kind="ine" label="Ver INE" /></p></div><fieldset><legend className="mb-2 text-sm font-semibold">Cantidad de vendajes</legend><div className="grid grid-cols-4 gap-2">{presets.map((preset) => <button className={`button ${quantity === preset ? "button-primary" : "button-secondary"}`} type="button" key={preset} onClick={() => setQuantity(preset)}>{preset}</button>)}</div><input className="field-input mt-3" value={quantity} onChange={(event) => setQuantity(event.target.value)} maxLength={32} placeholder="Personalizado, ej. 10x10" /></fieldset>{message && <p className="text-sm text-red-300">{message}</p>}<button className="button button-primary" type="submit" disabled={status === "sending"}>{status === "sending" ? "Registrando…" : "Registrar entrega"}</button></form>}</div>;
}
